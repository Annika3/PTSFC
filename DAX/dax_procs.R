...
compute_return <- function(y, type = "log", 
  ...
  if (type == "log"){
    ret <- c(rep(NA, h), 100*(log(y2)-log(y1))) #die ersten h Elemente werden mit NA befüllt
  } else {
...
#Berechnung Quantile Score
qs <- function(x, y, tau){
  ((1-tau)*(x-y)*(x > y) + tau*(y-x)*(y >= x))
}

eval_helper <- function(pred_mat, y){ #pred_mat=Eine Matrix mit vorhergesagten Werten für verschiedene Quantile
  sc <- qs(pred_mat[,1], y, .025) + 
    qs(pred_mat[,2], y, .25) + 
    qs(pred_mat[,3], y, .5) + 
    qs(pred_mat[,4], y, .75) + 
    qs(pred_mat[,5], y, .975)
  mean(sc) #Durchschnitt der Summe der Quantil Scores 
}
